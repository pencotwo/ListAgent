...
## Bundled utility: `fix_xlsx_dimension.py`

A standalone script (not part of the Tauri app) that repairs `.xlsx` files missing the worksheet `<dimension>` record — a common issue with Google Sheets exports that breaks tools like `excel-mcp-server`. Usage:

```
py -3 fix_xlsx_dimension.py <file.xlsx> [more.xlsx ...]
```

It rewrites each file in place, preserving everything else byte-for-byte.
